...
parser.add_argument('--tstart',  type=float, default=0.1, required=False)
...
figs, ax = plt.subplots(4, n_windows, figsize=(n_windows*5, 12))
# %%
# clim = dict(kind='value', pos_lims=[0.5, 1, 1.5])
clim = dict(kind='value', lims=[2, 3.5, 5])
...
plt.savefig(f'{fname.figures_dir(subject=SUBJECT)}/grand_average_raw.pdf',
            bbox_inches='tight'
            )
# %%
...
# tmin, tmax = np.searchsorted(stcs_RW[0].times,[tmin, tmax])
# %% same as "fsaverage_morph"
